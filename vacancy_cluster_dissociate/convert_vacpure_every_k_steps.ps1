<#
convert_vacpure_every_k_steps.ps1

Find files named like `vac-pure.<step>.cfg` (or `vac-pure.<step>`) in a target directory,
select those where step % K == 0, convert each to LAMMPS dump using cart = H0_diag*(frac-0.5),
and write outputs to a `corrected_s1` subdirectory under the target directory.

Usage examples:
  # default step=1000
  powershell -ExecutionPolicy Bypass -File .\convert_vacpure_every_k_steps.ps1 -dir "C:\path\to\s1\s1"

  # custom step
  powershell -ExecutionPolicy Bypass -File .\convert_vacpure_every_k_steps.ps1 -dir "C:\...\s1\s1" -step 500
...
$outDir = Join-Path $dir 'corrected_s1'
if (-not (Test-Path $outDir)) { New-Item -Path $outDir -ItemType Directory | Out-Null }

$files = Get-ChildItem -Path $dir -File | Where-Object { $_.Name -match '^vac-pure\.(\d+)(?:\.cfg)?$' }
if (-not $files -or $files.Count -eq 0) { Write-Host "No vac-pure files found in $dir" -ForegroundColor Yellow; return }

$processed = @()
foreach ($f in $files) {
    $m = [regex]::Match($f.Name, '^vac-pure\.(\d+)(?:\.cfg)?$')
    ...
    # parse numeric triples, take last N
    $coords = @()
